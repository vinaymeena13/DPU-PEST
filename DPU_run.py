...
##                    print(f"Solved: C_S={solution[0]:.6e}, C_R={solution[1]:.6e}, C_St={solution[2]:.6e}, C_L={solution[3]:.6e}, C_F={solution[4]:.6e}")
        ...
        def solve_system(self, initial_conditions: Dict, dt: float, total_time: float) -> Tuple[np.ndarray, np.ndarray]:
            ...
            n_steps = len(t_array)
            concentrations = np.zeros((n_steps, 5))

            concentrations[0] = [
                initial_conditions['C_S'], initial_conditions['C_R'],
                initial_conditions['C_St'], initial_conditions['C_L'], initial_conditions['C_F']
            ]
        ...
        def plot_results(self, t_array: np.ndarray, concentrations: np.ndarray, save_path: str = None):
            ...
            for i, (ax, comp, color) in enumerate(zip(axes, compartments, colors)):
                ...
                ax.set_yscale('log')
                ax.set_ylabel(f'{comp} Concentration (mg/kg)')
                ax.grid(True, ls='--', alpha=0.5)
                if i == 0:
                    ax.set_title('Plant Compartment Concentrations')
                if i == 4:
        ...
        def plot_mass_results(self, t_array: np.ndarray, save_path: str = None):
            ...
            for comp, name in zip(compartments, names):
                mass = np.array([self.calculate_mass(t, comp) for t in t_array])
                plt.plot(t_array, mass, label=name)
            plt.xlabel('Time (days)')
            plt.ylabel('Mass (kg)')
            plt.title('Plant Compartment Mass Growth')
            plt.legend()
            plt.grid(True)
            if save_path:
                plt.savefig(save_path, dpi=300, bbox_inches='tight')
        ...
        def run_simulation(self, initial_conditions: Dict = None, dt: float = None, total_time: float = None,
                           save_csv: bool = True, plot_filename: str = None, plot_mass: bool = True,
            ...
            if plot_mass and mass_plot_filename:
                self.plot_mass_results(t_array, mass_plot_filename)
            if save_csv:
                self.save_to_csv(t_array, concentrations)
            if save_mass_csv:
                self.save_mass_to_csv(t_array)

            return t_array, concentrations

    # ====================== MAIN EXECUTION ======================
##    print("PLANT CONCENTRATION TRANSPORT MODEL - GSA Mode")
    solver = PlantConcentrationSolver(csv_file='inputs.csv', time_varying_file='time_varying_data.csv')
    ...
    if os.path.exists(exp_file):
        ...
        Modelled_F = np.column_stack((t_array, modelled_fruits))
...
##        print("output.dat successfully created.")
        ...
        print("Warning: Exp.out not found.")
        return np.column_stack((t_array, results[:, 4]))
